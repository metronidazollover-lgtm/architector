...
6. **Навигационная модель Папок / Проводника (Folder Explorer Paradigm):**
   * При нахождении в контексте `currentContext` на холсте рендерятся исключительно его прямые дочерние объекты (`getRelativeDepth === 1`). Убраны размытые фоновые призраки предков (`opacity-20`, `blur`).
   * Просвечивание потомков и предков настраивается динамическими степперами X-Ray (`xRayDown`, `xRayUp`), верхний предел которых вычисляется функцией `HierarchyUtils.getMaxRelativeDepths(currentContext, ...)`.
   * Навигация управляется интерактивной Адресной строкой пути (Address Bar) на холсте и кликами по узлам-папкам (`📁 N объектов`).

7. **Порядок скриптов в `index.html`:**
...
8. **Визуализация портов и сетей связей:**
   * Стандартный размер порта: `w-3 h-3` (12px).
   * При выделении или подсвечивании соединенной сети порт масштабируется с коэффициентом `scale-[2.1]` (аккуратный размер ~25px, оптимизированный от громоздкого `scale-[3]`).

9. **Встроенная панель действий узла (Quick Action Strip):**
   * Панель скрыта по умолчанию и не занимает места на невыделенных узлах.
   * Открывается строго при одиночном клике мыши на узел (нажали и отпустили без перемещения).
   * При перетаскивании узла или отпускании на новой позиции панель не отображается.

10. **Динамический авторесайз и перенос текста (Live Auto-Sizing):**
