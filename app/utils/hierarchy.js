...
// =============================================================================
// v14 (Фаза 2, «Отчеты, аудиты, планы/Lanes_v14/PLAN_V14_LANES.md» §3/§7.12).
// Дорожки/окна-наборы/рамки-множества вместо уровней и слоёв — см.
// docs/LANES_MODEL.md. Добавлено АДДИТИВНО, отдельным блоком:
//   - функции из списка «Новые» — под своими финальными именами, коллизий с
//     существующими именами нет;
//   - функции из списка «Меняются» — под ВРЕМЕННЫМИ именами с суффиксом V14
...
//     старые одноимённые функции выше НЕ ТРОГАЮТСЯ: их продолжают вызывать
//     живой (ещё не переписанный) reducer.js через normalizeLevelWindows/
//     migrateProjectEntitiesToV13, и компоненты Layer.js/LevelWindow.js —
//     вплоть до Фазы 4;
//   - функции из списка «Удаляются» НЕ удаляются физически до конца Фазы 4.
// Переименование v14-версий в финальные имена и физическое удаление старых
// функций — один коммит в конце Фазы 4, вместе с подключением migrateToV14
// (см. §7.11/§7.12 плана). До этого момента весь блок ниже работает ТОЛЬКО
// с v14-состоянием ({ nodes, frames, windows, ports, links }), собранным
// напрямую (в тестах — через HierarchyUtils.parseNotation), а не через
...
// («глаз») дорожка — полоска. Метрики шапки/рамки окна — те же значения, что
// LEVEL_WINDOW_METRICS выше, но отдельная константа: WINDOW_METRICS не имеет
// отношения к levelWindows и не должна разъезжаться с ним при будущих правках.
const LANE_W = 420;
