function Toolbar() {
    ...
    const handleLoadDemo = () => {
        ...
        fetch('demo_project.json')
            .then(res => {
                if (!res.ok) throw new Error('Не удалось подгрузить demo_project.json');
                return res.json();
            })
            .then(data => {
                dispatch({ type: 'LOAD_STATE', payload: data });
            ...
            .catch(err => {
                console.error('Ошибка загрузки демонстрационного проекта из JSON:', err);
                alert('Не удалось автоматически загрузить файл demo_project.json. Убедитесь, что сервер запущен.');
            });
    };
